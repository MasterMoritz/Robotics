    ...
    private void stateMachine(State start) {
        ...
        while(stateMachineRunning)
            switch (state) {
                ...
                case SEARCH_BALL:
                    ...
                    for (int i = 0; i < 8; i++) {
                        ...
                        if (ballDetected()) {
                            ...
                            Log.v(TAG, "ball at: " + ball);
                            break;
                        }
                        //no ball in sight, turn 45 degrees and try again
                        else {
                            Log.v(TAG, "no ball detected, turning");
                            try {
                                sleep(100);
                            } catch (InterruptedException e) {
                                e.printStackTrace();
                ...
                case BALL_TO_TARGET:
                	Log.v(TAG, "BALL_TO_TARGET");
                	robot.turnToGoal();
                	Log.v(TAG, "turned to goal");
