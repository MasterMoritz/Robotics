...
public class BeaconDetector {
    ...
    private int compareContours(Point[] triple1, Point[] triple2) {
        ...
        * if one diameter is less than 75% of the other, the contours do not belong to one beacon*/
        double diameter1 = Math.abs(triple1[1].x - triple1[2].x);
        double diameter2 = Math.abs(triple2[1].x - triple2[2].x);
        if (diameter1/diameter2 < 0.75 || diameter2/diameter1 < 0.75) return 0;

        /* TODO check if the contours adjoin */

        /* check if they are in the same x area */
        double diameterX = diameter1/ 2;
        double x = Math.abs(triple2[1].x - triple2[2].x) / 2 + triple2[1].x;
        if (x > (triple1[1].x - diameterX) && x < (triple1[1].x + 2 * diameterX))
